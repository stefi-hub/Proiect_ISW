  ...
  return (
      <div className="guitarspage">
          ...
          <div className="guitar-section">
              {[0, 1].map(row => (
                  <div key={row} className="guitar-row">
                      {guitars.slice(row * 3, (row + 1) * 3).map(guitar => (
                          <div key={guitar.name} className="guitar-column">
                          <img src={guitar.src} alt={guitar.name} className="guitar-image"/>
                              <div className="guitar-name">
                                  <Link to={`/product/${guitar.name.toLowerCase()}`} className="guitar-link">
                                      {guitar.name}
                                  </Link>
                              </div>
                          </div>
